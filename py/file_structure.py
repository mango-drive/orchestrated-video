...
session_dir = f"video{sep}session"       # videos used to make the final edit
output_dir = f"video{sep}session{sep}output" # dir for final video edits
...
def resize_video(video_path, scale= '720:480'):
    ...
    target_path = path_in_same_dir(video_path, f"r_{name}") 
...
def prepare_session():
    ...
    videos = list_videos_in(video_dir)
    print(videos)
    for i, video_path in enumerate(videos):
        print("Resize ", video_path)
        resized_path = resize_video(video_path)
